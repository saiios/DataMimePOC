...
class DataManager {
 ...
 func fetchList(pageNumber: Int, complitionHandler: @escaping(_ response:[MySubmissionsModel]) -> Void) {
    ...
    func fetchEachBusinessDetails(with wid:String, complitionHandler: @escaping(_ response: BusinessDetailModel) -> Void) {
        // Check network connectivity
        if isConnectedToInternet() {

        let todoEndpoint: String = "https://securedev.datamime.com/fos/api/v1/listings/7a090d53-6ea8-402a-9d2e-6a6bb115741f"
            ...
            AF.request(todoEndpoint, headers:self.headers).responseJSON { response in
                ...
                guard response.data != nil else {
                       ...
                       return
                           ...
                           do {
                            let responseData = try JSONDecoder().decode(BusinessDetailModel.self, from: response.data!)
                            complitionHandler (responseData)
                           } catch {
                               print(error.localizedDescription)
                           }
